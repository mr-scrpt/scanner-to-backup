...
const readDir = async (from, archive) => {
  const cylceDir = async (start, archive) => {
    try {
      ...
      for await (const it of list) {
        ...
        if (stats && stats.isFile()) {
          ...
          await cylceDir(targetPath, archive);
        }
      }
    } catch (err) {}
  };

  await cylceDir(from, archive);
};
